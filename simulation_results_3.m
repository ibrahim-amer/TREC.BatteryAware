...
for n=1:N
    for m = 1:M
        try
            ...
            sim3.result1.rel_scores(n, m) = result1{n,m}.dataObj.workers_tasks_rel_prop * x;
            sim3.result1.Ms(n,m) = result1{n,m}.dataObj.M;
            x_axis = sim3.result1.Ms(n,m);
            y_axis = sim3.result1.rel_scores(n, m);
            
            strresult1 = strcat(strresult1, '(', num2str(x_axis),', ', num2str(y_axis), ')');

            x = result2{n,m}.all_sims{1}.x;
            sim3.result2.rel_scores(n, m) = result2{n,m}.dataObj.workers_tasks_rel_prop * x;
            sim3.result2.Ms(n,m) = result2{n,m}.dataObj.M;
            
            x_axis = sim3.result2.Ms(n,m);
            y_axis = sim3.result2.rel_scores(n, m);
...
%% Displaying x: tasks, y: reliability score for LCN method
disp('%% Displaying x: tasks, y: reliability score for LCN method');
disp(strresult1);
%% Displaying x: tasks, y: reliability score for dropped constraint method
disp('%% Displaying x: tasks, y: reliability score for dropped constraint method');
disp(strresult2);
